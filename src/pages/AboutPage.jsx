  ...
  return (
    <section className="relative overflow-hidden gradient-mesh th-bg-page">
      <div className="container-max section-padding !pb-12 text-center">
        <p className="text-sm font-semibold text-copper-500 mb-3">About Us</p>
      ...
      >
        ...
        <div className="rounded-2xl overflow-hidden mb-8 h-56 relative">
          <img
            ...
            className="w-full h-full object-cover"
          />
          <div className="absolute inset-0 bg-iron-900/50" />
      ...
      <div className="container-max">
        ...
        >
          <div className="relative">
            {/* Vertical line */}
            <div className="absolute left-[27px] top-0 bottom-0 w-0.5 bg-copper-100 dark:bg-copper-500/20" />

            {TIMELINE.map((item, i) => (
              <div key={item.year} className="relative flex gap-6 mb-10 last:mb-0">
                {/* Circle on the line */}
                <div className="relative z-10 w-14 h-14 shrink-0 rounded-full bg-copper-500 flex items-center justify-center shadow-lg shadow-copper-500/20">
...
function StatsSection() {
  const [ref, isInView] = useInView();
  return (
    <section className="section-padding bg-copper-500">
      <div
        ref={ref}
        className={`container-max transition-all duration-700 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
      >
